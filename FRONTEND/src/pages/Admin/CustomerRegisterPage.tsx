...
import { parseNextarCustomers, readNextarRows } from "@/utils/nextarImport";
...
export default function CustomerRegisterPage() {
  ...
  const [importingNex, setImportingNex] = useState(false);
  const nexFileRef = useRef<HTMLInputElement | null>(null);
  ...
  const importNexCustomers = async (file: File) => {
    setImportingNex(true);
    try {
      const parsed = parseNextarCustomers(await readNextarRows(file));
      if (parsed.records.length === 0) {
        throw new Error("Nenhum cliente válido foi encontrado no arquivo.");
      }

      const currentCustomers = await customerService.list();
      const byDocument = new Map(
        currentCustomers
          .filter((customer) => onlyDigits(customer.document))
          .map((customer) => [onlyDigits(customer.document), customer]),
      );
      ...
      const byNameAndPhone = new Map(
        currentCustomers
          .filter((customer) => onlyDigits(customer.cellphone || customer.telephone))
          .map((customer) => [
            customerIdentity(customer.customerName, customer.cellphone || customer.telephone),
            customer,
          ]),
      );
      let created = 0;
      ...
      for (const imported of parsed.records) {
        ...
        const existing =
          (imported.document ? byDocument.get(imported.document) : undefined) ||
          (imported.email ? byEmail.get(imported.email) : undefined) ||
          (onlyDigits(importedPhone)
            ? byNameAndPhone.get(customerIdentity(imported.name, importedPhone))
            : undefined);
        const birthDate = imported.birthDate || existing?.birthDate || "";
        const age = birthDate ? getAgeFromBirthDate(birthDate) : null;
        const payload: CustomerFormData = {
          customerName: imported.name,
          document: imported.document || existing?.document || "",
          birthDate,
          age: age === null ? existing?.age || "" : String(age),
          cep: imported.cep || existing?.cep || "",
          city: imported.city || existing?.city || "",
          state: imported.state || existing?.state || "",
          address: imported.address || existing?.address || "",
          neighborhood: imported.neighborhood || existing?.neighborhood || "",
          streetComplement: imported.complement || existing?.streetComplement || "",
          number: imported.number || existing?.number || "",
          referencePoint: existing?.referencePoint || "Importado do Nex",
          telephone: imported.telephone || existing?.telephone || "",
          cellphone: imported.cellphone || existing?.cellphone || imported.telephone || "",
          email: imported.email || existing?.email || "",
        };

        try {
          const saved = existing
            ? await customerService.update(existing.id, payload)
            : await customerService.create(payload);
          if (saved.document) byDocument.set(onlyDigits(saved.document), saved);
          if (saved.email) byEmail.set(saved.email.toLowerCase(), saved);
          byNameAndPhone.set(
            customerIdentity(saved.customerName, saved.cellphone || saved.telephone),
            saved,
          );
      ...
      Toast.error(error instanceof Error ? error.message : "Erro ao importar clientes do Nex.");
    } finally {
      setImportingNex(false);
      if (nexFileRef.current) nexFileRef.current.value = "";
    }
  };
  ...
  return (
    <PageLayout className="space-y-4 py-4 md:space-y-6 md:py-6 lg:py-8">
      <PageHeader
        ...
        description="Cadastre manualmente ou importe sua lista de clientes do Nex."
        action={
          <div className="flex flex-wrap gap-2">
            <input
              ...
              onChange={(event) => {
                ...
                if (file) void importNexCustomers(file);
              }}
            />
            <button
              type="button"
              onClick={() => nexFileRef.current?.click()}
              disabled={importingNex}
              className="btn-outline-secondary inline-flex items-center gap-2 disabled:opacity-60"
              title="Importar clientes exportados pelo Nex em Excel, CSV ou TXT"
            >
              <Upload size={16} />
              {importingNex ? "Importando..." : "Importar clientes do Nex"}
            </button>
...
function customerIdentity(name: string, phone: string) {
  return `${name.trim().toLowerCase()}|${onlyDigits(phone)}`;
}
